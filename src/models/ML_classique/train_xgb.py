...
from sklearn.metrics import balanced_accuracy_score, classification_report, confusion_matrix, f1_score, precision_score, recall_score
...
def plot_fold(model, fold_idx, test_part, test_sess, save_dir,
    ...
    fig.suptitle(
        f"XGB  |  Fold {fold_idx} — P{test_part} S{test_sess}"
    ...
    axes[0].barh([FEATURE_NAMES[i] for i in idx_sorted], importances[idx_sorted], color="#E53935")
...
def optuna_objective(trial, df, num_classes):
    params = {
        ...
        "n_estimators":      trial.suggest_int("n_estimators",      *XGB_OPTUNA_SPACE["n_estimators"]),
        "learning_rate":     trial.suggest_float("learning_rate",   *XGB_OPTUNA_SPACE["learning_rate"], log=True),
        "max_depth":         trial.suggest_int("max_depth",           *XGB_OPTUNA_SPACE["max_depth"]),
        "min_child_weight":  trial.suggest_int("min_child_weight",  *XGB_OPTUNA_SPACE["min_child_weight"]),
        "subsample":         trial.suggest_float("subsample",       *XGB_OPTUNA_SPACE["subsample"]),
        "colsample_bytree":  trial.suggest_float("colsample_bytree",*XGB_OPTUNA_SPACE["colsample_bytree"]),
        "reg_alpha":         trial.suggest_float("reg_alpha",       *XGB_OPTUNA_SPACE["reg_alpha"], log=True),
        "reg_lambda":        trial.suggest_float("reg_lambda",      *XGB_OPTUNA_SPACE["reg_lambda"], log=True),
        "gamma":             trial.suggest_float("gamma",           *XGB_OPTUNA_SPACE["gamma"]),
        "n_jobs": -1,
        "random_state": 42,
        "verbosity": 0,
        "early_stopping_rounds": 20,  # ← AJOUTÉ pour XGB 3.x
    }
...
def optimize_hyperparams(df, num_classes, n_trials=20):
    print(f"\nOPTUNA XGB — {n_trials} trials | {N_OPTUNA_SESSIONS} sessions\n" + "=" * 60)
    ...
    OPTUNA_PATH_XGB.parent.mkdir(parents=True, exist_ok=True)
    with open(OPTUNA_PATH_XGB, "w") as f:
        json.dump({"best_value": study.best_value, "best_params": study.best_params}, f, indent=4)
    return study.best_params


# ══════════════════════════════════════════════════════════════════════
# 8. UTILS — EXPORT ONNX → TFLite float32 + HEADER C
# ══════════════════════════════════════════════════════════════════════
def _export_onnx(model, X_sample, save_path):
    """Exporte un modèle XGB entraîné vers ONNX via onnxmltools."""
    import onnxmltools
...
def _export_tflite_float32(onnx_path, X_repr, save_dir, model_name):
    ...
    if not tflite_float32.exists():
        raise FileNotFoundError(f"TFLite float32 non généré: {tflite_float32}")
...
def _save_edge_comparison_metrics(model_name, edge_metrics):
    ...
    if model_name not in curr_metrics:
...
# 9. MODÈLE GLOBAL
# ══════════════════════════════════════════════════════════════════════
def train_global_model(df_labeled, df_unlabeled, xgb_params, num_classes):
    print("\n" + "=" * 60 + f"\nMODÈLE GLOBAL — {MODEL_NAME}\n" + "=" * 60)
    print(
        f"  Export ONNX direct pour stedgeai. "
    ...
    X_all = np.concatenate([X_tr, X_vl], axis=0)  # garde X_all pour compatibilite avec X_repr plus bas
    y_all = np.concatenate([y_tr, y_vl], axis=0)
    del df_unl, df_tr_raw, df_vl_raw
    gc.collect()
    ...
    try:
        ...
        model = build_model({**xgb_params, "early_stopping_rounds": 20})
        model.fit(
            X_tr, y_tr,
            sample_weight=sample_weights,
            eval_set=[(X_vl, y_vl)],
        ...
        # EXPORT : ONNX deployable stedgeai → Header C
        models_dir = MODELS_DIR / "XGB"
        ...
        # 3. Header C array depuis l'ONNX deployable par stedgeai
        _export_c_array_from_bytes(onnx_path, models_dir, f"{MODEL_NAME}_global")

        # 4. Comparaison métriques modèle natif vs ONNX
        edge_metrics = {}
        y_pred_native = model.predict(X_vl)
        edge_metrics["native_xgb"] = _compute_classification_metrics(y_vl, y_pred_native)
        edge_metrics["native_xgb"]["model_size_kb"] = round(pkl_path.stat().st_size / 1024, 2)
        ...
        fi_path = BASE_DIR / "training_curves" / "XGB" / "global_feature_importance.png"
...
def main():
    ...
    best_params = (
        ...
        if USE_OPTUNA_XGB
        else MODEL_PARAMS["XGB"]
    )

    # Préparer les params XGBoost finaux
    xgb_params = {
        "objective": "multi:softprob",
        "num_class": num_classes,
        "eval_metric": "mlogloss",
    ...
    import random
    random.seed(42)
    plots_dir   = BASE_DIR / "training_curves" / "XGB"
    models_dir  = MODELS_DIR / "XGB"
    ...
    train_global_model(df_labeled, df_unlabeled, xgb_params, num_classes)
