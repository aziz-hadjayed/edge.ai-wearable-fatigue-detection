...
def apply_smote(X, y, label="fit"):
    ...
    if k < 1:
        ...
        return X, y
    print(f"  SMOTE [{label}] avant : { {int(k): int(v) for k, v in zip(unique, counts)} }")
    ...
    print(f"  SMOTE [{label}] après : { {int(k): int(v) for k, v in zip(unique2, counts2)} }")
...
def optuna_objective(trial, df, num_classes):
    ...
    for fold_idx, (val_part, val_sess) in enumerate(val_sessions):
        ...
        try:
            ...
            if USE_SMOTE:
                X_train, y_train = apply_smote(X_train, y_train, label=f"opt P{val_part}S{val_sess}")
