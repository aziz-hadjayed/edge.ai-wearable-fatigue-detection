...
def plot_fold(history, fold_idx, test_part, test_sess, save_dir,
    ...
    fig.suptitle(
        f"CNN-LSTM  |  Fold {fold_idx} — P{test_part} S{test_sess}"
...
# ══════════════════════════════════════════════════════════════════════
# 7. MODÈLE CNN-LSTM + OPTUNA
# ══════════════════════════════════════════════════════════════════════
def build_model(trial, input_shape, num_classes):
    """
    Architecture CNN-LSTM :
      CNN Block  : N× Conv1D → BN → Act → MaxPool  (extraction de features locales)
    ...
    if isinstance(trial, optuna.trial.Trial) and not trial.user_attrs.get("params_printed", False):
        print(f"\nTrial {trial.number} — hyperparamètres Optuna :")
        print(json.dumps(trial.params, indent=2, sort_keys=True))
        trial.set_user_attr("params_printed", True)
    ...
    for i in range(n_conv_blocks):
        x = Conv1D(cnn_filters, cnn_kernel, padding="same", use_bias=False,
                   kernel_regularizer=l2(l2_reg))(x)
        x = BatchNormalization()(x)
        x = tf.keras.layers.Activation(act_name)(x)
        # MaxPool seulement si la séquence reste assez longue
        current_len = input_shape[0] // (pool_size ** (i + 1))
        if current_len > 1:
            x = MaxPooling1D(pool_size=pool_size)(x)

    # --- LSTM Temporal Modeling ---
    lstm_layer = LSTM(lstm_units, dropout=dropout_rate,
    ...
    # --- Classification Head ---
    x = Dense(dense_units, activation=act_name, kernel_regularizer=l2(l2_reg))(x)
...
def optimize_hyperparams(df, num_classes, n_trials=N_OPTUNA_TRIALS, n_optuna_epochs=N_OPTUNA_EPOCHS):
    ...
    OPTUNA_PATH_CNN_LSTM.parent.mkdir(parents=True, exist_ok=True)
...
def _apply_pruning_and_finetune(model, X_tr, y_tr, X_vl, y_vl, weight_dict,
    ...
    def _clone_for_pruning(layer):
        # Conv1D et Dense sont prunables. Les autres couches (LSTM,
        # Bidirectional, BatchNormalization, MaxPooling1D, Activation,
        # Dropout, InputLayer) sont reconstruites via from_config pour
        # éviter tout partage d'objet avec le modèle original — voir
        # _clone_model_for_tflite() dans ce même fichier pour le même
        # principe déjà appliqué.
    ...
    model_prunable = tf.keras.models.clone_model(
        model, clone_function=_clone_for_pruning
    )
    ...
    pruning_wrapper_cls = getattr(tfmot.sparsity.keras, "PruneLowMagnitude", None)
    for orig_layer, cloned_layer in zip(model.layers, model_prunable.layers):
        is_pruning_wrapper = (
            isinstance(cloned_layer, pruning_wrapper_cls)
            if pruning_wrapper_cls is not None
            else cloned_layer.__class__.__name__ == "PruneLowMagnitude" and hasattr(cloned_layer, "layer")
        )
        if is_pruning_wrapper:
...
def train_global_model(df_labeled, df_unlabeled, best_params, num_classes):
    ...
    # Split 90/10 par SESSION (pas par fenêtre), pour éviter toute fuite
    # participant/session entre train et val
    unique_sessions_all = [
    ...
    train_sessions_g = [unique_sessions_all[i] for i in shuffled[:split_point]]
    val_sessions_g   = [unique_sessions_all[i] for i in shuffled[split_point:]]

    df_tr_raw = df_labeled[
        df_labeled.set_index([COL_PARTICIPANT, COL_SESSION]).index.isin(train_sessions_g)
    ].copy()
    df_vl_raw = df_labeled[
        df_labeled.set_index([COL_PARTICIPANT, COL_SESSION]).index.isin(val_sessions_g)
    ].copy()

    # SMOTE seulement sur le train, avant le scaling et l'extraction de fenêtres
    ...
    X_tr, y_tr = extract_all_windows(df_tr_raw, W_SIZE, S_SIZE)
    X_vl, y_vl = extract_all_windows(df_vl_raw, W_SIZE, S_SIZE)
    X_unlabeled, _ = (
    ...
    gc.collect()
    print(f"  Fenêtres labellisées : train={len(X_tr)} val={len(X_vl)} | unlabeled : {len(X_unlabeled)}")
    ...
    try:
        ...
        # --- Étape 2 : pruning (Conv1D + Dense), avec repli propre en cas d'échec ---
        model_for_export = model
        try:
            print("  Pruning magnitude (Conv1D + Dense) : fine-tuning...")
        ...
        _save_tflite_int8(model_for_export, X_repr, models_dir, f"{MODEL_NAME}_global")
...
def main():
    ...
    best_params = (
        ...
        if USE_OPTUNA_CNN_LSTM
        else MODEL_PARAMS["CNN_LSTM"]
    )
    ...
    for test_idx, (test_part, test_sess) in enumerate(unique_sessions):
        ...
        # SMOTE seulement sur df_fit, avant le scaling et l'extraction de fenêtres
